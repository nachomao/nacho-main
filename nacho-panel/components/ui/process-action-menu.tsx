...
import { Check, EllipsisVertical, Leaf, Loader2, RotateCw } from "lucide-react"
...
type Props = {
  ...
  onAction: (action: ProcessMenuAction, enabled?: boolean) => void
}
...
const terminateMorphDurationMs = 1000

type TerminatePhase = "idle" | "morphing" | "armed"
...
function useTerminateMorph(onConfirm: () => void) {
  const [phase, setPhase] = useState<TerminatePhase>("idle")
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPhase("armed"), reducedMotion ? 0 : terminateMorphDurationMs)
    return () => window.clearTimeout(timer)
  }, [phase, reducedMotion])

  function activate() {
    if (phase === "idle") setPhase("morphing")
    else if (phase === "armed") onConfirm()
  }

  return { activate, morphed: phase !== "idle", ready: phase === "armed", reducedMotion }
}

function AnimatedTerminateIcon({ busy, morphed, reducedMotion }: { busy: boolean; morphed: boolean; reducedMotion: boolean }) {
...
function TerminateItemContent({ busy, morphed, reason, reducedMotion }: { busy: boolean; morphed: boolean; reason: string | null; reducedMotion: boolean }) {
  const label = morphed ? "确认终止" : "终止进程"
  return <>
    <AnimatedTerminateIcon busy={busy} morphed={morphed} reducedMotion={reducedMotion} />
...
function ContextItems({ process, supported, busyAction, onAction }: Props) {
  ...
  const terminate = useTerminateMorph(() => onAction("terminate"))
  return <>
    <Header process={process} supported={supported} />
    <ContextMenu.Separator className="mx-1 my-1 h-px bg-border" />
    <ContextMenu.Item label={terminate.morphed ? "确认终止" : "终止进程"} closeOnClick={terminate.ready} className={cn(itemCls, "text-negative")} disabled={busy || Boolean(terminateReason)} onClick={terminate.activate}><TerminateItemContent busy={busyAction === "terminate"} morphed={terminate.morphed} reason={terminateReason} reducedMotion={terminate.reducedMotion} /></ContextMenu.Item>
    <ContextMenu.Item className={itemCls} disabled={busy || Boolean(restartReason)} onClick={() => onAction("restart")}><BusyIcon active={busyAction === "restart"} icon={<RotateCw className="h-4 w-4 shrink-0" />} /><ItemLabel title="重启进程" reason={restartReason} /></ContextMenu.Item>
    <ContextMenu.Separator className="mx-1 my-1 h-px bg-border" />
    <ContextMenu.CheckboxItem className={itemCls} checked={process.efficiencyMode === true} disabled={busy || Boolean(efficiencyReason)} closeOnClick onCheckedChange={(checked) => onAction("efficiency", checked)}><BusyIcon active={busyAction === "efficiency"} icon={<Leaf className="h-4 w-4 shrink-0 text-positive" />} /><ItemLabel title="效能模式" reason={efficiencyReason} /><span className="ml-auto flex h-4 w-4 items-center justify-center">{process.efficiencyMode && <Check className="h-4 w-4" />}</span></ContextMenu.CheckboxItem>
  </>
}
...
function MobileItems(props: Props) {
  ...
  return <>
    ...
    <Menu.Item className={itemCls} disabled={busy || Boolean(restartReason)} onClick={() => onAction("restart")}><BusyIcon active={busyAction === "restart"} icon={<RotateCw className="h-4 w-4 shrink-0" />} /><ItemLabel title="重启进程" reason={restartReason} /></Menu.Item>
    <Menu.Separator className="mx-1 my-1 h-px bg-border" />
    <Menu.CheckboxItem className={itemCls} checked={process.efficiencyMode === true} disabled={busy || Boolean(efficiencyReason)} closeOnClick onCheckedChange={(checked) => onAction("efficiency", checked)}><BusyIcon active={busyAction === "efficiency"} icon={<Leaf className="h-4 w-4 shrink-0 text-positive" />} /><ItemLabel title="效能模式" reason={efficiencyReason} /><span className="ml-auto">{process.efficiencyMode && <Check className="h-4 w-4" />}</span></Menu.CheckboxItem>
  </>
}
